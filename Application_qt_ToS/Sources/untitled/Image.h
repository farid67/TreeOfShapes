...
public:
    ...
    int* computeTreeOfShapes(TreeType t);// fonction qui est censé calculer l'arbre de parenté de l'image courante en suivant l'agorithme de l'article

    int* computeTOS_perso(TreeType t); // fonction qui compute le TOS mais en modifiant l'ordre des étapes de l'algorithme
    ...
    void writeNode_simpl(Node* n,int proportion); // même fonction que pour AfficheNode, mais on écrit le contenu du noeud dans une image

    void writeTree_simpl(Tree* t, int proportion); // pareil que pour writeNode mais sur un arbre entier

    // écriture du graphique dans un fichier .dot

    void writeNode_graphviz(Node*n, const std::string& filename, int proportion);

    void writeTree_graphviz(Tree* t,const std::string& filename,int proportion);

    // écriture de info du noeud en début de graph
    void writeNodeInfo_graphviz(Node* t, const std::string& filename, int proportion);
    void writeTreeInfo_graphviz(Tree* t, const std::string& filename, int proportion);
